...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* --- Simulation of Automation 8: Looker Studio Dashboard --- */
  const runBtn8 = document.getElementById('run-automation-btn-8');
  const terminalBody8 = document.getElementById('terminal-body-8');

  if (runBtn8 && terminalBody8) {
    let isRunning8 = false;
    let originalPayload8 = terminalBody8.innerHTML;

    runBtn8.addEventListener('click', () => {
      if (isRunning8) return;

      if (runBtn8.textContent === 'Re-iniciar Simulación') {
        terminalBody8.innerHTML = originalPayload8;
        runBtn8.textContent = 'Acceder al Dashboard';
        return;
      }

      isRunning8 = true;
      runBtn8.style.opacity = '0.7';
      runBtn8.textContent = 'Sincronizando Datos...';
      runBtn8.style.cursor = 'wait';

      terminalBody8.innerHTML = '<span class="json-loading">> [CRM] Extrayendo datos de clientes y ventas...<br>> [Data Processing] Calculando KPIs: clientes redes sociales, conversiones por campaña...</span>';

      setTimeout(() => {
        terminalBody8.innerHTML = '<span class="json-loading">> [CRM] Extrayendo datos de clientes y ventas...<br>> [Data Processing] Calculando KPIs: clientes redes sociales, conversiones por campaña...<br>> [Geocoding] Procesando ubicaciones de clientes para mapa de calor...<br>> [Looker Studio] Actualizando dashboard con datos en tiempo real...<br>> [Visualization] Generando gráficos de ventas por campaña y beneficios...</span>';
      }, 1500);

      setTimeout(() => {
        terminalBody8.innerHTML = `<pre><code class="language-json" style="color: #E6E6E6;">{
  <span class="json-key">"dashboard_status"</span>: <span class="json-string">"UPDATED_SUCCESSFULLY"</span>,
  <span class="json-key">"kpi_updates"</span>: {
    <span class="json-key">"clientes_redes_sociales_mes"</span>: <span class="json-number">245</span>,
    <span class="json-key">"tasa_conversion_instagram"</span>: <span class="json-number">0.12</span>,
    <span class="json-key">"ventas_campana_blackfriday"</span>: <span class="json-number">45678.90</span>,
    <span class="json-key">"beneficio_promedio_cliente"</span>: <span class="json-number">125.50</span>
  },
  <span class="json-key">"geolocalizacion_data"</span>: {
    <span class="json-key">"clientes_mapeados"</span>: <span class="json-number">892</span>,
    <span class="json-key">"zonas_calor_principales"</span>: [<span class="json-string">"Madrid Centro"</span>, <span class="json-string">"Barcelona"</span>, <span class="json-string">"Valencia"</span>]
  },
  <span class="json-key">"last_sync"</span>: <span class="json-string">"2026-03-27T14:30:00Z"</span>
}</code></pre>`;
        runBtn8.textContent = 'Dashboard Actualizado';
        runBtn8.style.opacity = '1';
        runBtn8.style.cursor = 'pointer';

        setTimeout(() => {
          isRunning8 = false;
          runBtn8.textContent = 'Re-iniciar Simulación';
        }, 2000);
      }, 3500);
    });
  }
